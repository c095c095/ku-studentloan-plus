...
injectMainContent(newLayout, oldContent.body);
...
function createLayout(scrapeDOM) {
  ...
  activities.forEach(({ text, onclick }) => {
    ...
    link.onclick = () => {
      const w = window.open(url, 'popup_name', 'width=1250,height=900');
      if (w) w.focus();
    };
...
function injectMainContent(layout, oldBody = null) {
  const extractTableData = (target) => {
    if (target == null){ return; }

    const table = oldBody.querySelector('table');
    ...
    const headerMapping = {
      ...
      "สถานที่": "location",
      "รูปเอกสาร": "documentImage",
      "ชั่วโมงกิจกรรม": "activityHours",
      "วันที่จัด": "eventDate",
      "ประเภท": "type",
      "สถานะ": "status",
      "ยืนยัน": "confirm",
    ...
    const headers = Array.from(table.querySelectorAll('tr:first-child td'))
      .map(header => headerMapping[header.textContent.trim()] || header.textContent.trim());
    ...
    const data = Array.from(rows).slice(1).map(row => {
      ...
      return headers.reduce((obj, header, index) => {
        ...
        if (img) value = img.src;

        const button = cell.querySelector('button');
        if (button) value = button.getAttribute('onclick');

        const link = cell.querySelector('a');
        if (link && !img) value = link.href;
  ...
  const data = extractTableData(oldBody);
  ...
  mainContent.innerHTML = `
    <main class="flex flex-1 flex-col">
      <section class="border-b border-dashed">
        <div style="margin-left: auto; margin-right: auto; width: 100%; border-style: dashed;border-color: hsl(240 5.9% 90%/.7); border-left-width: 1px; border-right-width: 1px;">
          <div class="container flex flex-col items-start gap-1 py-8 md:py-10 lg:py-12">
            <h1 class="text-3xl font-bold leading-tight tracking-tighter md:text-4xl lg:leading-tight">ตรวจสอบการบันทึกกิจกรรม</h1>
            <p class="max-w-2xl text-lg font-light text-foreground">กรุณาตรวจสอบข้อมูลชั่วโมงกิจกรรมที่คุณได้บันทึกไว้อย่างละเอียดก่อนกดส่ง เพื่อให้เจ้าหน้าที่ กยศ ทำการตรวจสอบ</p>
          </div>
          <div class="container">
            <!-- -->
